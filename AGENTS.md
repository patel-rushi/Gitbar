...
- Never fan out one search request per person. GitHub's Search API ORs a repeated qualifier, so all authors go in one query (`author:a author:b …`), all teams in one (`team-review-requested:…`), all orgs in one (`org:a org:b`). Split a batch only on a 422 (query rejected), never on a 403/429 rate limit, which splitting would only make worse.
- Per-PR enrichment (reviews, pipeline, comments) is keyed on the PR's `updated_at`, which every search result already returns. A PR whose timestamp hasn't moved cannot have new activity, so repeat polls skip it for free. Never cache a failed lookup, only a successful one, or a transient error becomes sticky until the PR next changes.
- Review Requested vocabulary: an **author** entry means "open PRs that person opened"; a **team** entry means "PRs where that team's review is requested"; **your own login** is the exception and means "PRs that request you by name" (`user-review-requested:`), never "PRs you wrote". Your own entry therefore needs its own query group, since a different qualifier ANDs rather than ORs with `author:`. With no filters the tab falls back to `review-requested:<you>`, which GitHub defines as requests aimed at you **or** at a team you belong to (verified: 4 direct + 40 team-routed on a real account). Any filter replaces that default outright. UI says "author"/"you"/"team"; internals stay `reviewRequestedFilter` / user-vs-team by the `/` in the slug.
- The menu bar counter is the number of visible, actionable Review Requested PRs, in every mode: the no-filter default is itself an actionable queue. Derive it from the cached Review Requested list and local ignored state, including on startup and ignore/unignore actions; never add a GitHub request just to update the counter.
